...
**Prompt tối ưu cho Emotion Tagger:**

```python
# PROMPT FOR EMOTION API
# Model: openai/gpt-oss-20b (Best for speed and accuracy balance)

system_prompt_emotion = """You are an ultra-fast JSON API that detects the emotion in a robot's response.
...
def get_emotion(user_last_message: str, pika_response: str) -> dict:
    ...
    Returns a dictionary e.g., {"emotion_name": "worry"}
    """
...
"""
    ...
    chat_completion = client.chat.completions.create(
        ...
        temperature=0.0,
        max_tokens=50,  # JSON output is short
    ...
    # It's good practice to parse the JSON string from the response
    import json
    try:
        return json.loads(chat_completion.choices[0].message.content)
    except (json.JSONDecodeError, KeyError):
        return {"emotion_name": "idle"} # Fallback
...
# Expected Output: {'emotion_name': 'encouraging'}
```
...
**Prompt tối ưu cho Celebrate Detector:**

```python
# PROMPT FOR CELEBRATE API
# Model: openai/gpt-oss-20b

system_prompt_celebrate = """You are an ultra-fast JSON API, a binary classifier for a robot's celebration action.
...
def get_celebrate_decision(user_last_message: str, pika_response: str) -> dict:
    ...
    Returns a dictionary e.g., {"celebrate": "yes"}
    """
    start_time = time.perf_counter()
...
4.  **JSON Mode:** `response_format={"type": "json_object"}` là một tính năng cực kỳ mạnh mẽ, đảm bảo đầu ra luôn là một JSON hợp lệ, giúp bạn không cần phải xử lý lỗi định dạng.
